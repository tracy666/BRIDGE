...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_BRIDGE_single_organ_for_{args.st_embedding_folder_name}_of_retrieval_size{args.retrieval_size}_and_weighted_method_{weighted_method}"
    # Set up logger
    ...
    for row in tqdm(valid_df.itertuples(), total=len(valid_df)):
        ...
        single_organ_sc_dataset_embedding_folder_path = os.path.join(
        ...
        assert os.path.exists(single_organ_sc_dataset_embedding_folder_path) 
        ...
        single_organ_training_single_cell_gene_embedding, single_organ_training_single_cell_gene_ground_truth_expression = get_single_cell_gene_embedding_of_specific_organ_type_for_retrieval(            
            sc_dataset_embedding_folder_path=single_organ_sc_dataset_embedding_folder_path,
            main_data_storage=args.main_data_storage,
        ...
        single_organ_full_gene_slide_pcc, single_organ_full_gene_slide_genes_pcc_dict, single_organ_full_gene_all_test_gene_prediction_from_retrieval = compute_avg_pcc_given_indices_for_retrieval(
            all_train_gene_ground_truth_expression=single_organ_training_single_cell_gene_ground_truth_expression,
