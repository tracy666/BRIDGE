...
def get_retrieval_indexes(
    ...
    whole_distance, whole_indices = kdtree.query(all_test_image_embedding, k=retrieval_size)
...
def main():
    ...
    for row in tqdm(valid_df.itertuples(), total=len(valid_df)):
        ...
        multi_organ_testing_image_embedding, multi_organ_testing_gene_embedding = get_direct_partial_image_gene_embedding_for_retrieval(
            whole_dataset_embedding_folder_path=multi_organ_whole_dataset_embedding_folder_path,
            selected_index_list=testing_index_list,
        )
        
        retrieval_indexes_start_time = time.time()
        multi_organ_whole_distance, multi_organ_whole_indices = get_retrieval_indexes(
            all_train_gene_embedding=multi_organ_training_single_cell_gene_embedding,
            all_test_image_embedding=multi_organ_testing_image_embedding,
        ...
        multi_organ_retrieval_distance_saving_path = os.path.join(multi_organ_pred_data_folder_saving_path, f"{patient_dir_without_slash}_whole_distance.npy")
        multi_organ_retrieval_indices_saving_path = os.path.join(multi_organ_pred_data_folder_saving_path, f"{patient_dir_without_slash}_whole_indices.npy")
        if not os.path.exists(multi_organ_retrieval_distance_saving_path):
            np.save(multi_organ_retrieval_distance_saving_path, multi_organ_whole_distance)
            print(f"{row.patient_dir} multi organ whole distance saved")
        if not os.path.exists(multi_organ_retrieval_indices_saving_path):
            np.save(multi_organ_retrieval_indices_saving_path, multi_organ_whole_indices)
            print(f"{row.patient_dir} multi organ whole indices saved")
