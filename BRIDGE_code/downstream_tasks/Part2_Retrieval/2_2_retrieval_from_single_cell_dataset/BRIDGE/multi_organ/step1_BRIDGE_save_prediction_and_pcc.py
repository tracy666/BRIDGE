...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_BRIDGE_multi_organ_for_{args.st_embedding_folder_name}_of_retrieval_size{args.retrieval_size}_and_weighted_method_{weighted_method}"
    # Set up logger
    ...
    logger.info(f"Current evaluation multi-organ folder is {args.st_embedding_folder_name}")
    ...
    for row in tqdm(valid_df.itertuples(), total=len(valid_df)):
        ...
        multi_organ_whole_distance_file_path = os.path.join(
            multi_organ_pred_data_folder_saving_path,
            f"{patient_dir_without_slash}_whole_distance.npy",
        )
        multi_organ_whole_indices_file_path = os.path.join(
            multi_organ_pred_data_folder_saving_path,
            f"{patient_dir_without_slash}_whole_indices.npy",
        )
        assert os.path.exists(multi_organ_whole_distance_file_path) and os.path.exists(multi_organ_whole_indices_file_path)

        multi_organ_whole_distance = np.load(multi_organ_whole_distance_file_path, allow_pickle=True)
        multi_organ_whole_indices = np.load(multi_organ_whole_indices_file_path, allow_pickle=True)

        multi_organ_sc_dataset_embedding_folder_path = os.path.join(
            retrieval_saving_dir,
            "2_0_save_single_cell_dataset_embedding",
            "BRIDGE",
            args.st_embedding_folder_name,
        )
        assert os.path.exists(multi_organ_sc_dataset_embedding_folder_path)
        
        multi_organ_whole_dataset_embedding_folder_path = os.path.join(
            retrieval_saving_dir,
            "2_0_save_whole_st_training_data_embedding",
            "BRIDGE",
            args.st_embedding_folder_name,
        )
        assert os.path.exists(multi_organ_whole_dataset_embedding_folder_path)

        multi_organ_training_single_cell_gene_embedding, multi_organ_training_single_cell_gene_ground_truth_expression = get_single_cell_gene_embedding_of_specific_organ_type_for_retrieval(            
            sc_dataset_embedding_folder_path=multi_organ_sc_dataset_embedding_folder_path,
            main_data_storage=args.main_data_storage,
            raw_data_folder_name=args.raw_data_folder_name,
            organ_type=row.organ_type,
        )

        print(f"Start with multi organ pcc calculation")
        multi_organ_full_gene_pcc_start_time = time.time()
        multi_organ_full_gene_slide_pcc, multi_organ_full_gene_slide_genes_pcc_dict, multi_organ_full_gene_all_test_gene_prediction_from_retrieval = compute_avg_pcc_given_indices_for_retrieval(
            all_train_gene_ground_truth_expression=multi_organ_training_single_cell_gene_ground_truth_expression,
            all_test_gene_ground_truth_expression=whole_dataset_gene_ground_truth_expression[testing_index_list],
            whole_distance=multi_organ_whole_distance,
            whole_indices=multi_organ_whole_indices,
            retrieval_size=args.retrieval_size,
            selected_genes_list=selected_genes_list,
            eval_selected_genes_list=selected_genes_list,
            weighted=weighted,
        )
        multi_organ_full_gene_pcc_end_time = time.time()
        print(f"Finished calculating multi organ full gene pcc, takes {multi_organ_full_gene_pcc_end_time - multi_organ_full_gene_pcc_start_time} seconds")

        multi_organ_eval_gene_slide_pcc_start_time = time.time()
        multi_organ_eval_gene_slide_pcc, multi_organ_eval_gene_slide_genes_pcc_dict, multi_organ_eval_gene_all_test_gene_prediction_from_retrieval = compute_avg_pcc_given_indices_for_retrieval(
            all_train_gene_ground_truth_expression=multi_organ_training_single_cell_gene_ground_truth_expression,
            all_test_gene_ground_truth_expression=whole_dataset_gene_ground_truth_expression[testing_index_list],
            whole_distance=multi_organ_whole_distance,
            whole_indices=multi_organ_whole_indices,
        ...
        multi_organ_eval_gene_slide_pcc_end_time = time.time()
        print(f"Finished calculating multi organ eval gene pcc, takes {multi_organ_eval_gene_slide_pcc_end_time - multi_organ_eval_gene_slide_pcc_start_time} seconds")
        ...
        multi_organ_retrieval_saving_path = os.path.join(
            multi_organ_pred_data_folder_saving_path,
            f"weighted_method_{weighted_method}",
        )
        os.makedirs(multi_organ_retrieval_saving_path, exist_ok=True)

        multi_organ_ground_truth_saving_path = os.path.join(multi_organ_retrieval_saving_path, f"{patient_dir_without_slash}_ground_truth.npy")
        multi_organ_prediction_saving_path = os.path.join(multi_organ_retrieval_saving_path, f"{patient_dir_without_slash}_prediction.npy")
        multi_organ_pcc_dict_saving_path = os.path.join(multi_organ_retrieval_saving_path, f"{patient_dir_without_slash}_pcc_dict.npy")
        if not os.path.exists(multi_organ_ground_truth_saving_path):
            np.save(multi_organ_ground_truth_saving_path, whole_dataset_gene_ground_truth_expression[testing_index_list])
            print(f"{row.patient_dir} multi organ ground truth saved")
        if not os.path.exists(multi_organ_prediction_saving_path):
            np.save(multi_organ_prediction_saving_path, multi_organ_full_gene_all_test_gene_prediction_from_retrieval)
