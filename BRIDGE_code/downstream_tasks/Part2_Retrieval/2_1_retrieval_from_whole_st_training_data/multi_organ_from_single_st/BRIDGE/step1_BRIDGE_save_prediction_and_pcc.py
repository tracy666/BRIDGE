...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_BRIDGE_multi_organ_for_{args.st_embedding_folder_name}_of_retrieval_size_{args.retrieval_size}_and_weighted_method_{weighted_method}"
    # Set up logger
    ...
    for row in tqdm(valid_df.itertuples(), total=len(valid_df)):
        ...
        single_organ_training_index_list = get_index_list(
        ...
        multi_organ_full_gene_slide_pcc, multi_organ_full_gene_slide_genes_pcc_dict, multi_organ_full_gene_all_test_gene_prediction_from_retrieval = compute_avg_pcc_given_indices_for_retrieval(
            all_train_gene_ground_truth_expression=whole_dataset_gene_ground_truth_expression[single_organ_training_index_list],
