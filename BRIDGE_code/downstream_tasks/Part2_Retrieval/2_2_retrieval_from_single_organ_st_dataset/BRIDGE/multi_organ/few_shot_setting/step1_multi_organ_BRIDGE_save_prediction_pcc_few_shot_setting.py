...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_multi_organ_BRIDGE_retrieval_from_single_organ_ST_dataset_for_{args.model_settings}"
    # Set up logger
    ...
    for row in tqdm(few_shot_df.itertuples(), total=len(few_shot_df)):
        ...
        logger.info(f"For {row.patient_dir} with {row.organ_type}, multi-organ BRIDGE biomarker gene performance: {biomarker_gene_slide_pcc}")
        ...
        if not os.path.exists(ground_truth_saving_path):
            ...
            print(f"{row.patient_dir}: multi-organ BRIDGE retrieval ground truth saved to {ground_truth_saving_path}")
        ...
        if not os.path.exists(prediction_saving_path):
            ...
            print(f"{row.patient_dir}: multi-organ BRIDGE retrieval prediction saved to {prediction_saving_path}")
        ...
        if not os.path.exists(pcc_dict_saving_path):
            ...
            print(f"{row.patient_dir}: multi-organ BRIDGE retrieval pcc dict saved to {pcc_dict_saving_path}")

    biomarker_gene_pcc_avg = sum(biomarker_gene_slide_pcc_list)/len(biomarker_gene_slide_pcc_list)
    logger.info(f"For multi-organ BRIDGE under {args.model_settings}, Average biomarker gene performance: {biomarker_gene_pcc_avg}")
