...
def main():
    ...
    for row in tqdm(few_shot_df.itertuples(), total=len(few_shot_df)):
        ...
        # BLEEP_training_image_embedding, BLEEP_training_gene_embedding = get_direct_partial_image_gene_embedding_for_retrieval(
        #     whole_dataset_embedding_folder_path=BLEEP_ST_reference_dataset_embedding_folder_path,
        #     organ_type=row.organ_type,
        #     selected_index_list=single_organ_training_index_list,
        # )
        BLEEP_testing_image_embedding, BLEEP_testing_gene_embedding = get_direct_partial_image_gene_embedding_for_retrieval(
            whole_dataset_embedding_folder_path=BLEEP_ST_reference_dataset_embedding_folder_path,
            organ_type=row.organ_type,
        ...
        BLEEP_whole_distance, BLEEP_whole_indices = get_retrieval_indexes(
            all_train_gene_embedding=BLEEP_training_single_cell_gene_embedding,
            all_test_image_embedding=BLEEP_testing_image_embedding,
        ...
        print(f"Start saving single-organ BLEEP distance and indices")
        ...
        if not os.path.exists(distance_saving_path):
            np.save(distance_saving_path, BLEEP_whole_distance)
            print(f"{row.patient_dir}: BLEEP retrieval distance saved to {distance_saving_path}")
        ...
        if not os.path.exists(indices_saving_path):
            np.save(indices_saving_path, BLEEP_whole_indices)
            print(f"{row.patient_dir}: BLEEP retrieval indices saved to {indices_saving_path}")
