...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_multi_organ_BRIDGE_retrieval_from_SC_dataset_for_{args.model_settings}"
    # Set up logger
    ...
    for row in tqdm(few_shot_df.itertuples(), total=len(few_shot_df)):
        ...
        full_gene_slide_pcc, full_gene_slide_genes_pcc_dict, full_gene_all_test_gene_prediction_from_retrieval = compute_avg_pcc_given_indices_for_retrieval(
            ...
            all_train_gene_ground_truth_expression=multi_organ_BRIDGE_training_single_cell_gene_ground_truth_expression,
