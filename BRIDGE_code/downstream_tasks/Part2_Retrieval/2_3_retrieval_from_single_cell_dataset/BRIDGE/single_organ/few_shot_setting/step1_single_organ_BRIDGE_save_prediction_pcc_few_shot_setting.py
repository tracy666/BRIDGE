...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_single_organ_BRIDGE_retrieval_from_SC_dataset_for_{args.model_settings}"
    # Set up logger
    ...
    for row in tqdm(few_shot_df.itertuples(), total=len(few_shot_df)):
        ...
        full_gene_slide_pcc, full_gene_slide_genes_pcc_dict, full_gene_all_test_gene_prediction_from_retrieval = compute_avg_pcc_given_indices_for_retrieval(
            ...
            all_train_gene_ground_truth_expression=single_organ_BRIDGE_training_single_cell_gene_ground_truth_expression,
        ...
        logger.info(f"For {row.patient_dir} with {row.organ_type}, single-organ BRIDGE biomarker gene performance: {biomarker_gene_slide_pcc}")
        ...
        if not os.path.exists(ground_truth_saving_path):
            ...
            print(f"{row.patient_dir}: single-organ BRIDGE retrieval ground truth saved to {ground_truth_saving_path}")
        ...
        if not os.path.exists(prediction_saving_path):
            ...
            print(f"{row.patient_dir}: single-organ BRIDGE retrieval prediction saved to {prediction_saving_path}")
        ...
        if not os.path.exists(pcc_dict_saving_path):
            ...
            print(f"{row.patient_dir}: single-organ BRIDGE retrieval pcc dict saved to {pcc_dict_saving_path}")

    biomarker_gene_pcc_avg = sum(biomarker_gene_slide_pcc_list)/len(biomarker_gene_slide_pcc_list)
    logger.info(f"For single-organ BRIDGE under {args.model_settings}, Average biomarker gene performance: {biomarker_gene_pcc_avg}")
