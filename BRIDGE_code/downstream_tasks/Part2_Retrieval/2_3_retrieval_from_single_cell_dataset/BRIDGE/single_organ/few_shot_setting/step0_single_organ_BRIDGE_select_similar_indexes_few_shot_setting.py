...
def main():
    ...
    few_shot_df_path = os.path.join(args.working_codespace, "generated_files", args.generation_date, f"few_shot_df.csv")
    few_shot_df = pd.read_csv(few_shot_df_path)

    for row in tqdm(few_shot_df.itertuples(), total=len(few_shot_df)):
        ...
        single_organ_BRIDGE_ST_reference_dataset_embedding_folder_path = os.path.join(
        ...
        assert os.path.exists(single_organ_BRIDGE_ST_reference_dataset_embedding_folder_path)
        
        single_organ_BRIDGE_SC_reference_dataset_embedding_folder_path = os.path.join(
            downstream_tasks_folder,
            f"Part2_Retrieval",
        ...
        assert os.path.exists(single_organ_BRIDGE_SC_reference_dataset_embedding_folder_path)
        
        single_organ_BRIDGE_training_single_cell_gene_embedding, single_organ_BRIDGE_training_single_cell_gene_ground_truth_expression = get_single_cell_gene_embedding_of_specific_organ_type_for_retrieval(            
            sc_dataset_embedding_folder_path=single_organ_BRIDGE_SC_reference_dataset_embedding_folder_path,
            main_data_storage=args.main_data_storage,
            raw_data_folder_name=args.raw_data_folder_name,
            organ_type=row.organ_type,
        )

        single_organ_BRIDGE_testing_image_embedding, single_organ_BRIDGE_testing_gene_embedding = get_direct_partial_image_gene_embedding_for_retrieval(
            whole_dataset_embedding_folder_path=single_organ_BRIDGE_ST_reference_dataset_embedding_folder_path,
            organ_type="multi_organ",
            selected_index_list=testing_index_list,
        )

        retrieval_indices_start_time = time.time()
        single_organ_BRIDGE_whole_distance, single_organ_BRIDGE_whole_indices = get_retrieval_indexes(
            all_train_gene_embedding=single_organ_BRIDGE_training_single_cell_gene_embedding,
            all_test_image_embedding=single_organ_BRIDGE_testing_image_embedding,
        ...
        print(f"Start saving single-organ BRIDGE distance and indices")
        ...
        if not os.path.exists(distance_saving_path):
            np.save(distance_saving_path, single_organ_BRIDGE_whole_distance)
            print(f"{row.patient_dir}: single-organ BRIDGE retrieval distance saved to {distance_saving_path}")
        ...
        if not os.path.exists(indices_saving_path):
            np.save(indices_saving_path, single_organ_BRIDGE_whole_indices)
            print(f"{row.patient_dir}: single-organ BRIDGE retrieval indices saved to {indices_saving_path}")
