...
def get_image_gene_embedding_for_retrieval(
    ...
    all_gene_ground_truth_expression_list = list()
    ...
    for _, batch in tqdm(enumerate(dataloader), total=len(dataloader), desc='Extract gene sampling pool embeddings for retrieval'):
        ...
        if gene_encoder.model_name == "TabNet" or gene_encoder.model_name == "MLP":
            original_gene_feature = gene_encoder(input_gene_expression.float())
            original_gene_embedding = gene_encoder.forward_head(original_gene_feature)
            original_gene_embedding = F.normalize(original_gene_embedding, p=2, dim=1)
        elif gene_encoder.model_name == "scGPT":
            ...
            with torch.cuda.amp.autocast(enabled=True): # Automatic mixed precision
                original_gene_feature, _ = gene_encoder(
                    [
            ...
            raise ValueError(f"Unsupported gene encoder model: {gene_encoder.model_name}")
        all_image_embedding_list.append(original_image_embedding.detach().cpu().numpy())
        all_gene_ground_truth_expression_list.append(input_gene_expression.detach().cpu().numpy())
    ...
    all_gene_ground_truth_expression = np.concatenate(all_gene_ground_truth_expression_list, axis=0)
    all_gene_embedding = np.concatenate(all_gene_embedding_list, axis=0)
    return all_image_embedding, all_gene_ground_truth_expression, all_gene_embedding
...
def main():
    ...
    parser.add_argument('--checkpoint_list', type=str, nargs='*', help='List of checkpoints to evaluate')

    parser.add_argument('--model_settings', type=str, default="dim_256")
    parser.add_argument('--single_organ_info', type=str, default="all") # "brain"
    ...
    for checkpoint in args.checkpoint_list:
        ...
        if single_organ:
            st_embedding_saving_dir = os.path.join(
                ...
                f"{args.model_settings}",
                f"{args.single_organ_info}_at_epoch_{epoch_number}"
            )
            os.makedirs(st_embedding_saving_dir, exist_ok=True)
            print(st_embedding_saving_dir)
        else:
            st_embedding_saving_dir = os.path.join(
                downstream_tasks_saving_dir,
        ...
        print(f"Evaluating BRIDGE model checkpoint: {checkpoint} with information {args.model_settings}")

        extraction_start_time = time.time()
        whole_dataset_image_embedding, whole_dataset_gene_ground_truth_expression, whole_dataset_gene_embedding = get_image_gene_embedding_for_retrieval(
            image_encoder=pretrained_BRIDGE_model.image_encoder,
            gene_encoder=pretrained_BRIDGE_model.gene_encoder,
            dataloader=whole_dataloader,
            device=device,
        )
        extraction_end_time = time.time()
        print(f"Extraction time: {running_time_display(extraction_end_time - extraction_start_time)}")

        saving_start_time = time.time()
        torch.save(torch.from_numpy(whole_dataset_image_embedding), os.path.join(st_embedding_saving_dir, "whole_st_dataset_image_embedding.pt"))
        torch.save(torch.from_numpy(whole_dataset_gene_embedding), os.path.join(st_embedding_saving_dir, "whole_st_dataset_gene_embedding.pt"))
        starting_end_time = time.time()
        print(f"Saving time: {running_time_display(starting_end_time - saving_start_time)}")
