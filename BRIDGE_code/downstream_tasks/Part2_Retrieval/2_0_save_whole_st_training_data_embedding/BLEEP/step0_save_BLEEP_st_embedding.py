...
def get_image_gene_embedding_for_retrieval(
    ...
    for _, batch in tqdm(enumerate(dataloader), total=len(dataloader), desc='Extract gene sampling pool embeddings for retrieval'):
        ...
        if model.image_encoder_name == "ResNet50":
        ...
        elif model.image_encoder_name == "CTransPath" or model.image_encoder_name == "DenseNet121":
            original_image_feature = model.image_encoder(original_patch_view)
...
def main():
    ...
    for checkpoint in args.checkpoint_list:
        ...
        print(f"Evaluating BLEEP model checkpoint: {checkpoint} with information {args.model_settings}")
