...
def main():
    ...
    for organ_type in [
        ...
        print(f"processing of {organ_type}...")
        checkpoint = os.path.join(args.main_data_storage, "finalized_checkpoints", "BRIDGE", "single_organ", f"BRIDGE_{organ_type}", "19.ckpt")
        ...
        print(f"Evaluating single-organ BRIDGE model checkpoint: {checkpoint} for organ {organ_type}")
        print("---------------------------------------------------------------------------------------------------------------")
        ...
        print(f"single-organ BRIDGE ST reference dataset extraction time: {running_time_display(ST_extraction_end_time - ST_extraction_start_time)}")
        print("---------------------------------------------------------------------------------------------------------------")
        ...
        torch.save(torch.from_numpy(whole_dataset_image_embedding), os.path.join(st_embedding_saving_dir, f"{organ_type}_whole_st_dataset_image_embedding.pt"))
        torch.save(torch.from_numpy(whole_dataset_gene_embedding), os.path.join(st_embedding_saving_dir, f"{organ_type}_whole_st_dataset_gene_embedding.pt"))
        starting_end_time = time.time()
        print(f"single-organ BRIDGE ST reference dataset saving time: {running_time_display(starting_end_time - saving_start_time)}")
        print("---------------------------------------------------------------------------------------------------------------")
