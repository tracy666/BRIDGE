...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_natural_image_pretraining_UNI_image_feature_for_dataset_{dataset_name}"
    # Set up logger
    ...
    for patient_dir in selected_patient_dir_list:
        logger.info(f"Single image modality - natural image pretraining - UNI image feature - dataset {dataset_name} - patient {patient_dir}")
