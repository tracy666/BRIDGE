...
from BRIDGE_model_linear_0824_modified import BRIDGEModel
...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_natural_image_pretraining_multi_organ_BRIDGE_image_feature_for_dataset_{dataset_name}"
    # Set up logger
    ...
    multi_organ_BRIDGE_checkpoint = "/data1/zliang/0000_0901_finalized_checkpoints/BRIDGE/multi_organ/fifth_version_6loss_DenseNet_TabNet_dim_128_BRIDGE_2024_08_29_07_40_36_all/49.ckpt"
    pretrained_BRIDGE_model = BRIDGEModel.load_from_checkpoint(
        multi_organ_BRIDGE_checkpoint,
        strict=False,
    ...
    multi_organ_BRIDGE_image_model = pretrained_BRIDGE_model.image_encoder
    ...
    for patient_dir in selected_patient_dir_list:
        logger.info(f"Single image modality - natural image pretraining - multi_organ_BRIDGE image feature - dataset {dataset_name} - patient {patient_dir}")
        ...
        single_slide_image_feature, single_slide_label_numerical = get_natural_image_pretrained_model_image_feature(
            image_model=multi_organ_BRIDGE_image_model,
            dataloader=dataloader,
            device=device,
        )
