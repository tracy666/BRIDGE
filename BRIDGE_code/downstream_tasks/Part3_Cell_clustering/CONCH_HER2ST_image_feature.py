...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_natural_image_pretraining_CONCH_image_feature_for_dataset_{dataset_name}"
    # Set up logger
    ...
    CONCH_image_model = ImageEncoder(
            model_name="CONCH",
    ...
    for patient_dir in selected_patient_dir_list:
        logger.info(f"Single image modality - natural image pretraining - CONCH image feature - dataset {dataset_name} - patient {patient_dir}")
