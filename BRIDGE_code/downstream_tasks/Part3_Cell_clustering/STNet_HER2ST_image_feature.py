...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_natural_image_pretraining_single_organ_STNet_image_feature_for_dataset_{dataset_name}"
    # Set up logger
    ...
    single_organ_STNet_checkpoint = "/data1/zliang/0000_0901_finalized_checkpoints/STNet/single_organ/STNet_2024_08_07_13_21_16_breast/19.ckpt"
    pretrained_STNet_model = STNetModel.load_from_checkpoint(
        single_organ_STNet_checkpoint,
        strict=False,
    ...
    if pretrained_STNet_model.image_encoder_name == "DenseNet121":
        pretrained_STNet_model.model.classifier = torch.nn.Identity()
        single_organ_STNet_image_model = pretrained_STNet_model.model
    elif (pretrained_STNet_model.image_encoder_name == "CTransPath") or (pretrained_STNet_model.image_encoder_name == "DINOv1") or (pretrained_STNet_model.image_encoder_name == "DINOv2"):
        single_organ_STNet_image_model = pretrained_STNet_model.image_encoder
    else:
        raise ValueError(f"pretrained_STNet_model.image_encoder_name={pretrained_STNet_model.image_encoder_name} is not supported")
    ...
    for patient_dir in selected_patient_dir_list:
        logger.info(f"Single image modality - natural image pretraining - single_organ_STNet image feature - dataset {dataset_name} - patient {patient_dir}")
        ...
        single_slide_image_feature, single_slide_label_numerical = get_natural_image_pretrained_model_image_feature(
            image_model=single_organ_STNet_image_model,
            dataloader=dataloader,
            device=device,
        )
