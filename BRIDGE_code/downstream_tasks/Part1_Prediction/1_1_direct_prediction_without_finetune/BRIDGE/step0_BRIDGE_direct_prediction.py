...
def main():
    ...
    parser.add_argument('--single_organ', action='store_true', help='Whether to use weighted sum for retrieval')
    args = parser.parse_args()

    if args.single_organ:
        single_organ = True
    else:
        single_organ = False
    print(f"single organ: {single_organ}")
    ...
    if single_organ:
        logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_fifth_version_BRIDGE_single_organ_for_{args.model_settings}"
        # Set up logger
        ...
        logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_fifth_version_BRIDGE_multi_organ_for_{args.model_settings}"
        # Set up logger
        logger = logging.getLogger(__name__) # Create a custom logger
        logger.setLevel(logging.INFO)
        # Create a file handler and set its level to INFO
    ...
    multi_organ_full_gene_slide_pcc_list = list()
    multi_organ_eval_gene_slide_pcc_list = list()
    single_organ_full_gene_slide_pcc_list = list()
    single_organ_eval_gene_slide_pcc_list = list()

    if not single_organ:
        multi_organ_checkpoint = "/disk1/zliang/0000_0901_finalized_checkpoints/BRIDGE/multi_organ/fifth_version_6loss_DenseNet_TabNet_dim_128_BRIDGE_2024_08_29_07_40_36_all/49.ckpt"
        logger.info(f"Multi-organ checkpoint path: {multi_organ_checkpoint}")
        
        try:
            multi_organ_epoch_number = int(multi_organ_checkpoint.split("/")[-1].split(".")[0]) + 1
        except ValueError:
            multi_organ_epoch_number = int((multi_organ_checkpoint.split("/")[-1].split(".")[0]).split("_")[-1]) + 1
            
        multi_organ_pretrained_BRIDGE_model = BRIDGEModel.load_from_checkpoint(
            multi_organ_checkpoint,
            strict=False,
            main_data_storage=args.main_data_storage,
            working_codespace=args.working_codespace,
            weight_decay=1e-3,
        )
        multi_organ_pretrained_BRIDGE_model.eval()
        
        for row in tqdm(valid_df.itertuples(), total=len(valid_df)):
            print(f"Processing {row.patient_dir} with {row.organ_type}")

            multi_organ_single_slide_eval_dataset = SingleSlideEvalImageGeneDataset(
                main_data_storage=args.main_data_storage,
            ...
            multi_organ_single_slide_eval_dataloader = torch.utils.data.DataLoader(
                multi_organ_single_slide_eval_dataset,
            ...
            multi_organ_single_slide_gene_ground_truth, multi_organ_single_slide_gene_prediction = get_ground_truth_and_prediction_gene_expressions(
                image_encoder=multi_organ_pretrained_BRIDGE_model.image_encoder,
                image_to_gene_decoder=multi_organ_pretrained_BRIDGE_model.image_to_gene_decoder,
                dataloader=multi_organ_single_slide_eval_dataloader,
                device=device,
            )
            multi_organ_full_gene_slide_pcc, multi_organ_full_gene_slide_genes_pcc_dict = compute_avg_pcc_given_genes(
                ground_truth_gene_expression=multi_organ_single_slide_gene_ground_truth,
                prediction_gene_expression=multi_organ_single_slide_gene_prediction,
                selected_genes_list=selected_genes_list,
                eval_selected_gene_names_list=selected_genes_list,
            )
            multi_organ_eval_gene_slide_pcc, multi_organ_eval_gene_slide_genes_pcc_dict = compute_avg_pcc_given_genes(
                ground_truth_gene_expression=multi_organ_single_slide_gene_ground_truth,
                prediction_gene_expression=multi_organ_single_slide_gene_prediction,
                selected_genes_list=selected_genes_list,
                eval_selected_gene_names_list=eval_selected_genes_list,
            )
            multi_organ_full_gene_slide_pcc_list.append(multi_organ_full_gene_slide_pcc)
            multi_organ_eval_gene_slide_pcc_list.append(multi_organ_eval_gene_slide_pcc)
            logger.info(f"For {row.patient_dir} with {row.organ_type}, multi organ full gene performance: {multi_organ_full_gene_slide_pcc}")
            logger.info(f"For {row.patient_dir} with {row.organ_type}, multi organ eval gene performance: {multi_organ_eval_gene_slide_pcc}")
            
            print(f"Start saving multi organ data")
            multi_organ_saving_start_time = time.time()
            multi_organ_full_gene_slide_genes_pcc_dict = {k: v for k, v in sorted(multi_organ_full_gene_slide_genes_pcc_dict.items(), key=lambda item: item[1], reverse = True)}
            patient_dir_without_slash = (row.patient_dir).replace("/", "_")
            
            multi_organ_pred_data_folder_saving_path = os.path.join(
                downstream_tasks_saving_dir,
                "multi_organ",
                f"{args.model_settings}_at_epoch_{multi_organ_epoch_number}",
                patient_dir_without_slash,
            )
            os.makedirs(multi_organ_pred_data_folder_saving_path, exist_ok=True)
            multi_organ_ground_truth_saving_path = os.path.join(multi_organ_pred_data_folder_saving_path, f"{patient_dir_without_slash}_ground_truth.npy")
            multi_organ_prediction_saving_path = os.path.join(multi_organ_pred_data_folder_saving_path, f"{patient_dir_without_slash}_prediction.npy")
            multi_organ_pcc_dict_saving_path = os.path.join(multi_organ_pred_data_folder_saving_path, f"{patient_dir_without_slash}_pcc_dict.npy")
            if not os.path.exists(multi_organ_ground_truth_saving_path):
                np.save(multi_organ_ground_truth_saving_path, multi_organ_single_slide_gene_ground_truth)
                print(f"{row.patient_dir} multi organ ground truth saved")
            if not os.path.exists(multi_organ_prediction_saving_path):
                np.save(multi_organ_prediction_saving_path, multi_organ_single_slide_gene_prediction)
                print(f"{row.patient_dir} multi organ prediction saved")
            if not os.path.exists(multi_organ_pcc_dict_saving_path):
                with open(multi_organ_pcc_dict_saving_path, "wb") as f:
                    pickle.dump(multi_organ_full_gene_slide_genes_pcc_dict, f)
                print(f"{row.patient_dir} multi organ pcc dict saved")
            multi_organ_saving_end_time = time.time()
            print(f"Finished saving multi organ data, takes {multi_organ_saving_end_time - multi_organ_saving_start_time} seconds")
            print(f"Multi organ all finished!!")
            
        assert len(multi_organ_eval_gene_slide_pcc_list) == len(multi_organ_full_gene_slide_pcc_list)  == len(valid_df)
        multi_organ_eval_gene_pcc_avg = sum(multi_organ_eval_gene_slide_pcc_list)/len(multi_organ_eval_gene_slide_pcc_list)
        multi_organ_full_gene_pcc_avg = sum(multi_organ_full_gene_slide_pcc_list)/len(multi_organ_full_gene_slide_pcc_list)

        logger.info(f"Avg eval gene multi organ pcc avg: {multi_organ_eval_gene_pcc_avg}")
        logger.info(f"Avg full gene multi organ pcc avg: {multi_organ_full_gene_pcc_avg}")
            
    else:
        # single organ
        print(f"Start with single organ")
        for row in tqdm(valid_df.itertuples(), total=len(valid_df)):
            print(f"Processing {row.patient_dir} with {row.organ_type}")
            
            if row.organ_type == "nose":
                single_organ_checkpoint = "/disk1/zliang/0000_0901_finalized_checkpoints/BRIDGE/single_organ/BRIDGE_2024_08_31_12_15_54_nose/19.ckpt"
            elif row.organ_type == "ovary":
                single_organ_checkpoint = "/disk1/zliang/0000_0901_finalized_checkpoints/BRIDGE/single_organ/BRIDGE_2024_08_31_11_59_05_ovary/19.ckpt"
            elif row.organ_type == "skin":
                single_organ_checkpoint = "/disk1/zliang/0000_0901_finalized_checkpoints/BRIDGE/single_organ/BRIDGE_2024_08_31_09_50_28_skin/19.ckpt"
            elif row.organ_type == "liver":
                single_organ_checkpoint = "/disk1/zliang/0000_0901_finalized_checkpoints/BRIDGE/single_organ/BRIDGE_2024_08_31_11_37_15_liver/19.ckpt"
            elif row.organ_type == "small_and_large_intestine":
                single_organ_checkpoint = "/disk1/zliang/0000_0901_finalized_checkpoints/BRIDGE/single_organ/BRIDGE_2024_08_31_07_30_45_small_and_large_intestine/19.ckpt"
            elif row.organ_type == "lung":
                single_organ_checkpoint = "/disk1/zliang/0000_0901_finalized_checkpoints/BRIDGE/single_organ/BRIDGE_2024_08_31_06_08_01_lung/19.ckpt"
            elif row.organ_type == "breast":
                single_organ_checkpoint = "/disk1/zliang/0000_0901_finalized_checkpoints/BRIDGE/single_organ/BRIDGE_2024_08_31_04_25_50_breast/19.ckpt"
            elif row.organ_type == "prostate":
                single_organ_checkpoint = "/disk1/zliang/0000_0901_finalized_checkpoints/BRIDGE/single_organ/BRIDGE_2024_08_30_21_44_32_prostate/19.ckpt"
            elif row.organ_type == "heart":
                single_organ_checkpoint = "/disk1/zliang/0000_0901_finalized_checkpoints/BRIDGE/single_organ/BRIDGE_2024_08_30_21_44_03_heart/19.ckpt"
            elif row.organ_type == "brain":
                single_organ_checkpoint = "/disk1/zliang/0000_0901_finalized_checkpoints/BRIDGE/single_organ/BRIDGE_2024_08_30_21_43_19_brain/19.ckpt"
            else:
                raise ValueError
            ...
            single_organ_pretrained_BRIDGE_model = BRIDGEModel.load_from_checkpoint(
            ...
            single_organ_pretrained_BRIDGE_model.eval()
            ...
            single_organ_single_slide_gene_ground_truth, single_organ_single_slide_gene_prediction = get_ground_truth_and_prediction_gene_expressions(
                image_encoder=single_organ_pretrained_BRIDGE_model.image_encoder,
                image_to_gene_decoder=single_organ_pretrained_BRIDGE_model.image_to_gene_decoder,
