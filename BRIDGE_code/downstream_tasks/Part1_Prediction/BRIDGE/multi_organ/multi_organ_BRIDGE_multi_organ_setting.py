...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_BRIDGE_multi_organ_for_{args.model_settings}"
    # Set up logger
    ...
    for row in tqdm(test_df.itertuples(), total=len(test_df)):
        ...
        logger.info(f"multi-organ checkpoint path: {checkpoint} for {row.patient_dir} with {row.organ_type}")
        ...
        print(f"Start saving multi-organ BRIDGE data")
        ...
        if not os.path.exists(ground_truth_saving_path):
            ...
            print(f"{row.patient_dir} multi-organ BRIDGE ground truth saved")
        if not os.path.exists(prediction_saving_path):
            np.save(prediction_saving_path, single_slide_gene_prediction)
            print(f"{row.patient_dir} multi-organ BRIDGE prediction saved")
        if not os.path.exists(pcc_dict_saving_path):
            ...
            print(f"{row.patient_dir} multi-organ BRIDGE pcc dict saved")
        saving_end_time = time.time()
        print(f"Finished saving multi-organ BRIDGE data, takes {saving_end_time - saving_start_time} seconds")
