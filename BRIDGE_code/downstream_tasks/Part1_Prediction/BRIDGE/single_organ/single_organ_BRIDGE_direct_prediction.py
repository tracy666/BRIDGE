...
def main():
    ...
    logger_name = datetime.now().strftime('%Y%m%d%H%M%S') + f"_BRIDGE_single_organ_for_{args.model_settings}"
    # Set up logger
    ...
    logger.info(f"HEG + HVG setting is of {args.num_of_eval_genes} HEGs and HVGs, in total {len(selected_genes_list)} involved.")
    ...
    single_organ_selected_gene_slide_pcc_list = list()
    single_organ_biomarker_gene_slide_pcc_list = list()
    
    for row in tqdm(test_df.itertuples(), total=len(test_df)):
        print(f"Processing {row.patient_dir} with {row.organ_type}")
        single_organ_checkpoint = os.path.join(args.main_data_storage, "finalized_checkpoints", "BRIDGE", "single_organ", f"BRIDGE_{row.organ_type}", "19.ckpt")
        ...
        single_organ_selected_gene_slide_pcc, single_organ_selected_gene_slide_genes_pcc_dict = compute_avg_pcc_given_genes(
            ground_truth_gene_expression=single_organ_single_slide_gene_ground_truth,
            prediction_gene_expression=single_organ_single_slide_gene_prediction,
            full_gene_names_list=full_genes_list,
            eval_selected_gene_names_list=selected_genes_list,
        )
        single_organ_biomarker_gene_slide_pcc, single_organ_biomarker_gene_slide_genes_pcc_dict = compute_avg_pcc_given_genes(
            ground_truth_gene_expression=single_organ_single_slide_gene_ground_truth,
            prediction_gene_expression=single_organ_single_slide_gene_prediction,
            full_gene_names_list=full_genes_list,
            eval_selected_gene_names_list=biomarker_genes_list,
        )
        single_organ_selected_gene_slide_pcc_list.append(single_organ_selected_gene_slide_pcc)
        single_organ_biomarker_gene_slide_pcc_list.append(single_organ_biomarker_gene_slide_pcc)
        logger.info(f"For {row.patient_dir} with {row.organ_type}, single organ biomarker gene performance: {single_organ_biomarker_gene_slide_pcc}")
    ...
    assert len(single_organ_selected_gene_slide_pcc_list) == len(single_organ_biomarker_gene_slide_pcc_list)  == len(test_df)
    single_organ_biomarker_gene_pcc_avg = sum(single_organ_biomarker_gene_slide_pcc_list)/len(test_df)
    logger.info(f"For {args.model_settings}, Average biomarker gene performance: {single_organ_biomarker_gene_pcc_avg}")
