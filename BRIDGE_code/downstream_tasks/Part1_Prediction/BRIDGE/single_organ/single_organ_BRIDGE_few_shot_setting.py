...
def main():
    ...
    for row in tqdm(test_df.itertuples(), total=len(test_df)):
        ...
        checkpoint = os.path.join(args.main_data_storage, "finalized_checkpoints", "BRIDGE", "single_organ", f"BRIDGE_{row.organ_type}", "19.ckpt")
        ...
        single_slide_gene_ground_truth, single_slide_gene_prediction = get_ground_truth_and_prediction_gene_expressions(
            ...
            image_to_gene_decoder=pretrained_BRIDGE_model.image_to_gene_decoder,
        ...
        print(f"Start saving single-organ BRIDGE data")
        ...
        if not os.path.exists(ground_truth_saving_path):
            ...
            print(f"{row.patient_dir} single-organ BRIDGE ground truth saved")
        if not os.path.exists(prediction_saving_path):
            np.save(prediction_saving_path, single_slide_gene_prediction)
            print(f"{row.patient_dir} single-organ BRIDGE prediction saved")
        if not os.path.exists(pcc_dict_saving_path):
            ...
            print(f"{row.patient_dir} single-organ BRIDGE pcc dict saved")
        saving_end_time = time.time()
        print(f"Finished saving single-organ BRIDGE data, takes {saving_end_time - saving_start_time} seconds")
