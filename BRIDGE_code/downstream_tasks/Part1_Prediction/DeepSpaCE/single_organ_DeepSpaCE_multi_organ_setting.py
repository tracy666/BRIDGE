...
def main():
    ...
    for row in tqdm(test_df.itertuples(), total=len(test_df)):
        ...
        checkpoint = os.path.join(args.main_data_storage, "finalized_checkpoints", "DeepSpaCE", "single_organ", f"DeepSpaCE_{row.organ_type}", "19.ckpt")
        ...
        pretrained_DeepSpaCE_model = DeepSpaCEModel.load_from_checkpoint(
            checkpoint,
            strict=False,
        ...
        pretrained_DeepSpaCE_model.eval()
        ...
        single_slide_gene_ground_truth, single_slide_gene_prediction = get_ground_truth_and_prediction_gene_expressions(
            image_encoder=pretrained_DeepSpaCE_model.image_encoder,
            image_to_gene_decoder=pretrained_DeepSpaCE_model.image_to_gene_decoder,
        ...
        logger.info(f"For {row.patient_dir} with {row.organ_type}, single-organ DeepSpaCE biomarker gene performance: {biomarker_gene_slide_pcc}")

        print(f"Start saving single-organ DeepSpaCE data")
        ...
        if not os.path.exists(ground_truth_saving_path):
            ...
            print(f"{row.patient_dir} single-organ DeepSpaCE ground truth saved")
        if not os.path.exists(prediction_saving_path):
            np.save(prediction_saving_path, single_slide_gene_prediction)
            print(f"{row.patient_dir} single-organ DeepSpaCE prediction saved")
        if not os.path.exists(pcc_dict_saving_path):
            ...
            print(f"{row.patient_dir} single-organ DeepSpaCE pcc dict saved")
        saving_end_time = time.time()
        print(f"Finished saving single-organ DeepSpaCE data, takes {saving_end_time - saving_start_time} seconds")
    ...
    logger.info(f"For single-organ DeepSpaCE under {args.model_settings}, Average biomarker gene performance: {biomarker_gene_pcc_avg}")
