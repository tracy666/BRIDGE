...
class ImageGeneDataset(Dataset):
    def __init__(
        ...
        valid_df_path = os.path.join(self.working_codespace, "generated_files", self.generation_date, f"valid_df.csv")
        ...
        valid_mask = np.isin(self.all_patient_dir_numerical, valid_patient_dir_numerical_list)
        ...
        for index, boolean_value in enumerate(valid_mask):
            if boolean_value:
        ...
        elif self.train_or_test == "valid":
            self.selected_index_list = valid_index_list
            self.train_image_transform1, self.train_image_transform2 = [get_image_transforms(train_or_test=self.train_or_test, image_size=224, crop_size=224)] * 2
